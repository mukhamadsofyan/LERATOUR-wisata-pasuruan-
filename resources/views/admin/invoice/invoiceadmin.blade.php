...
<style>
    ...
    .popup[data-popup="invoice"].is-active {
        display: flex !important;
        align-items: center;
        justify-content: center;
    }

    .popup[data-popup="invoice"] .popup__bg {
        position: absolute;
        inset: 0;
        background: rgba(0, 0, 0, .45);
    }

    .popup[data-popup="invoice"] .popup__content {
        position: relative;
        width: 100%;
        max-width: 700px;
        max-height: 90vh;
...
<body>
        ...
        <div class="dashboard -is-sidebar-visible js-dashboard">
            ...
            <div class="dashboard__content">
                ...
                <div class="dashboard__content_content">
                    ...
                    <h1 class="text-30">Invoice</h1>
                    <p class="">
                        Kelola dan pantau seluruh transaksi pemesanan wisata, lengkap dengan detail invoice dan metode
                        pembayaran.
                    </p>
                            ...
                            <div class="tabs__content js-tabs-content">
                                ...
                                <div class="tabs__pane -tab-item-1 is-tab-el-active">
                                    ...
                                    @if ($transaksis->hasPages())
                                        <div class="pagination justify-center mt-30">
                                            {{-- Previous --}}
    ...
    <!-- ================= POPUP INVOICE (WAJIB DI LUAR DASHBOARD) ================= -->
    <div class="popup js-popup" data-popup="invoice">
        <div class="popup__bg js-popup-close"></div>

        <div class="popup__content invoice-card">
            <!-- HEADER -->
            <div class="invoice-top">
                <div>
                    <h3>Detail Invoice</h3>
                    <p>Informasi transaksi pemesanan</p>
                </div>
                <div class="invoice-id" id="m_order">-</div>
            </div>
            ...
            <div class="invoice-grid">
                ...
                <div class="item">
                    ...
                    <strong id="m_harga">-</strong>
                </div>

                <div class="item">
                    <span>Jumlah</span>
                    <strong id="m_jumlah">-</strong>
                </div>

                <div class="item">
                    <span>Metode Pembayaran</span>
                    <strong id="m_metode">-</strong>
                </div>
            </div>
            ...
            <!-- TRANSACTION ID -->
            <div class="invoice-trx">
                Transaction ID: <span id="m_trx">-</span>
            </div>

            <!-- ACTION -->
            <div class="invoice-action">
                <button class="button -md -dark-1 js-popup-close">Tutup</button>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('click', function(e) {
            ...
            if (btn) {
                const popup = document.querySelector('.js-popup[data-popup="invoice"]');
                popup.classList.add('is-active');

                fetch(`/admin/invoice/${btn.dataset.id}`)
                    .then(res => res.json())
                    .then(data => {
                        document.getElementById('m_order').innerText = data.order_id;
                        document.getElementById('m_wisata').innerText = data.nama_wisata;
                        document.getElementById('m_nama').innerText = data.nama_pemesan;
                        document.getElementById('m_email').innerText = data.email;
                        document.getElementById('m_harga').innerText =
                            'Rp ' + Number(data.harga_tiket).toLocaleString();
                        document.getElementById('m_jumlah').innerText = data.jumlah_tiket;
                        document.getElementById('m_total').innerText =
                            'Rp ' + Number(data.total_bayar).toLocaleString();
                        document.getElementById('m_metode').innerText = data.payment_method ?? '-';
                        document.getElementById('m_trx').innerText = data.transaction_id ?? '-';

                        // STATUS BADGE
                        const statusEl = document.getElementById('m_status');
                        statusEl.innerText = data.payment_status;
                        statusEl.className = 'status-badge ' + data.payment_status;
                    });
            ...
            // ================= CLOSE POPUP =================
            if (e.target.closest('.js-popup-close') || e.target.classList.contains('popup__bg')) {
                document
                    .querySelectorAll('.popup.is-active')
                    .forEach(popup => popup.classList.remove('is-active'));
            }
        });
    </script>
