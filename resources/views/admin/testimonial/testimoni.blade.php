                            ...
                            <div class="tabs__controls row x-gap-40 y-gap-10 lg:x-gap-20 js-tabs-controls">
                                ...
                                <div class="col-auto">
                                    <button
                                        ...
                                        data-tab-target=".-tab-item-1">Testimonial</button>
                                </div>
                                ...
                                <div class="tabs__pane -tab-item-1 is-tab-el-active">
                                    <div class="overflowAuto">
                                            ...
                                            <tbody>
                                                @forelse ($testimonial as $item)
                                                    <tr>
                                                        ...
                                                        <td>{{ $item->notelp ?? '-' }}</td>
                                                        <td
                                                            style="max-width:500px;word-break:break-word;white-space:normal;">
                                                        ...
                                                        <td>
                                                            ...
                                                            @if ($item->foto_testimoni)
                                                                <img src="{{ asset('storage/testimoni/' . $item->foto_testimoni) }}"
                                                                    class="foto-thumb" alt="Foto Testimoni">
                                                            @else
                                                        ...
                                                        <td>
                                                            <div class="action-wrap">
                                                                ...
                                                                {{-- STATUS MENUNGGU --}}
                                                                @if (in_array(strtolower($item->status), ['tunggu', 'menunggu']))
                                                                    {{-- ACC --}}
                                                                    <form
                                                                        action="{{ route('admin.testimonial.accept', $item->id) }}"
                                                                        method="POST">
                                                                        @csrf
                                                                    ...
                                                                    {{-- REJECT --}}
                                                                    <form
                                                                        action="{{ route('admin.testimonial.reject', $item->id) }}"
                                                                        method="POST"
                                                                        onsubmit="return confirm('Yakin menolak testimoni ini?')">
                                                                        @csrf
                                                                ...
                                                                @if (in_array(strtolower($item->status), ['terima', 'tolak', 'diterima', 'ditolak']))
                                                                    {{-- HAPUS --}}
                                                                    <form id="delete-form-{{ $item->id }}"
                                                                        action="{{ route('admin.testimonial.delete', $item->id) }}"
                                                                        method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                        ...
                                        <div class="text-14 text-center mt-20">
                                            Testimoni yang berstatus <b>Diterima</b> akan ditampilkan di halaman utama
                                            website
                                        </div>
                                    </div>
                                ...
                                @if ($testimonial->hasPages())
                                    <div class="pagination justify-center mt-30">
                                        ...
                                        <div class="pagination__count">
                                            @foreach ($testimonial->links()->elements[0] as $page => $url)
                                                @if ($page == $testimonial->currentPage())
                                        ...
                                        {{-- NEXT --}}
                                        @if ($testimonial->hasMorePages())
                                            <a href="{{ $testimonial->nextPageUrl() }}"
                                    ...
                                    <div class="text-14 text-center mt-20">
                                        Showing {{ $testimonial->firstItem() }} – {{ $testimonial->lastItem() }}
                                        of {{ $testimonial->total() }} testimoni
                                    </div>
                                @endif
